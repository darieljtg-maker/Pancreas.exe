...
export function calcularPorcion({
  ...
  // ── 1. Plato base ──────────────────────────────────────────────────────
  // La ración del plan corregida SOLO por la glucosa actual.
  const ajustableBaseIdeal = receta.ajustable.carbos + ajuste;
  const ajustableBase = Math.max(0, ajustableBaseIdeal);
  const totalSinFactores = carbosFijos + ajustableBase;

  // ── 2. Castigos porcentuales ───────────────────────────────────────────
  // Alba e índice glucémico recortan el plato base. Como los alimentos fijos
  // no se tocan, el recorte completo lo absorbe el alimento ajustable, que
  // por eso puede quedarse en 0 antes de que el total llegue al objetivo.
  ...
  const ajustableTrasFactoresIdeal = totalPermitido - carbosFijos;
  const ajustableTrasFactores = Math.max(0, ajustableTrasFactoresIdeal);

  // ── 3. Colchón por tendencia ───────────────────────────────────────────
  // Se suma ÍNTEGRO al final y a propósito: cuando la flecha va hacia abajo
  // esos gramos son glucosa de rescate para una caída que ya está ocurriendo,
  // no parte de la ración del plan. Recortarlos con los porcentajes del alba
  // o del índice glucémico quitaría protección justo cuando más hace falta.
  const carbosAjustable = ajustableTrasFactores + extra;

  // Topado = la ración del plan llegó a cero, ya sea por glucosa alta o
  // porque los porcentajes recortaron por debajo de lo que aportan los fijos.
  // Si hay colchón por tendencia, los gramos finales aun así serán > 0.
  const topado = ajustableBaseIdeal < 0 || ajustableTrasFactoresIdeal < 0;
  ...
  return {
    ...
    alba,
    igAlto,
    // Cuántos gramos recortó cada factor, para poder mostrar la cuenta.
    ...
    totalTrasFactores: redondear(carbosFijos + ajustableTrasFactores, 1),
    topado,
    fijos: receta.fijos,
    carbosFijos,
