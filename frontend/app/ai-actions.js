...
const PROMPT_VIGILANTE =
  ...
  'los carbohidratos activos. Responde ÚNICAMENTE con una alerta corta (máx 2 ' +
  'líneas) si prevés una caída por debajo de 70 mg/dL o un pico arriba de 250 ' +
  "mg/dL en los próximos 40 minutos. Si todo está estable, responde 'Estable'.";
...
function formatearContextoVigilante({ lecturas, ultimaComida, insulinaReciente }) {
  const curva = lecturas
    .map((l) => `${hora(l.timestamp)}  ${l.glucose_value} mg/dL  ${l.trend_arrow || ''}`.trim())
    .join('\n');
